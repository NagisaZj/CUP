...
class Agent(AbstractAgent):
    ...
    def update_actor_and_alpha(
        ...
        if use_expert:
            ...
            # [0,1,2]
            # [18, 13, 8, 44, 31, 48, 35, 36, 49, 6, 38, 4, 23, 26, 24, 25, 5, 21, 37, 15] 236
            # [18, 13, 49, 8, 31, 36, 44, 26, 48, 7, 25, 35, 37, 4, 38, 6, 23, 15, 5, 24] 248
            # [18, 13, 49, 44, 8, 20, 24, 48, 21, 36, 23, 6, 26, 31, 25, 37, 14, 16, 15, 38, 5, 4, 7, 19, 35] 256
            # [18, 44, 8, 31, 13, 49, 48, 23, 36, 6, 26, 38, 15, 21, 16, 35, 24, 37, 5, 20, 14, 4, 19] 810
            # [44, 8, 18, 13, 49, 38, 6, 21, 23, 24, 48, 31, 7, 4, 16, 5, 37, 20, 15, 36, 14, 35, 26, 19, 46] 809
            source_list = [18, 13, 49, 44, 8, 20, 24, 48, 21, 36, 23, 6, 26, 31, 25, 37, 14, 16, 15, 38, 5, 4, 7, 19, 35]
            # candids = np.random.choice(source_list,3,replace=False)

            # for i in range(3):
            #     log_pi_e = torch.ones_like(mu).detach()
            #     log_pi_e = log_pi_e * -1.0
            #     pi_e = torch.randn_like(mu).detach()
            #     _, pi_e, _ = _squash(pi_e, pi_e, None)
            #     Q_e_1, Q_e_2 = self.critic_target(mtobs=mtobs, action=pi_e, detach_encoder=True)
            #     Q_e = torch.max(Q_e_1, Q_e_2).detach()
            #     for j in range(2):
            #         noise = torch.randn_like(mu)
            #         pi_e_n = pi_e + noise * log_pi_e.exp()
            #         _, pi_e_n, _ = _squash(pi_e_n, pi_e_n, None)
            #         Q_e_1_n, Q_e_2_n = self.critic_target(mtobs=mtobs, action=pi_e_n, detach_encoder=True)
            #         Q_e = Q_e + (torch.max(Q_e_1_n, Q_e_2_n)).detach()
            #     Q_e = Q_e / 3
            #     mu_e_all.append(pi_e)
            #     pi_e_all.append(pi_e)
            #     log_pi_e_all.append(log_pi_e)
            #     log_std_e_all.append(log_pi_e)
            #     Q_e_all.append(Q_e)
            ...
            #     kld_tmp = gaussian_kld(mu, log_std*1, pi_e,log_pi_e*1)
            #     kld_tmp = torch.mean(kld_tmp,1,keepdim=True)
            #     #print(kld_tmp.shape)
            #     kld_all.append(kld_tmp)
        # kld_all.append((gaussian_kld(mu_e, log_std_e * 2,mu, log_std * 2)))
            ...
            #         mu_e, pi_e, log_pi_e, log_std_e, mask, old_mu = self.source_agent.actor.forward_expert_auto(mtobs=infomtobs, detach_encoder=True,
            #                                                                           expert_id=i,
            #                                                                           relabel_id=relabel_id)
            #         # if i!=0:
            #         #     log_std_e = log_std_e * 2.5
            #         #     std_e = log_std_e.exp()
            #         #     noise = torch.randn_like(old_mu)
            #         #     pi_e = old_mu + noise * std_e
            #         #     # log_pi_e = _gaussian_logprob(noise, log_std)
            #         #
            #         #     _, pi_e, _ = _squash(old_mu, pi_e, None)
            #         # print('source')
            #     # else:
            ...
            #         for j in range(2):
            #             noise = torch.randn_like(old_mu)
            #             pi_e_n = old_mu + noise * std_e
            #             _, pi_e_n, _ = _squash(old_mu, pi_e_n, None)
            #             Q_e_1_n, Q_e_2_n = self.critic_target(mtobs=mtobs, action=pi_e_n, detach_encoder=True)
            #             Q_e = Q_e + (torch.max(Q_e_1_n, Q_e_2_n)).detach()
            #         Q_e = Q_e / 3
            ...
            #         mu_e, pi_e, log_pi_e, log_std_e, mask, old_mu = self.source_agent_2.actor.forward_expert_auto(mtobs=infomtobs, detach_encoder=True,
            #                                                                           expert_id=i,
            #                                                                           relabel_id=relabel_id)
            #         # print('source')
            #         # if i!=0:
            ...
            #         #     pi_e = old_mu + noise * std_e
            #         #     log_pi_e = _gaussian_logprob(noise, log_std)
            #         #     _, pi_e, _ = _squash(old_mu, pi_e, log_pi_e)
            #
            #         Q_e_1,Q_e_2 = self.critic_target(mtobs=mtobs, action=pi_e, detach_encoder=True)
            #         Q_e = torch.max(Q_e_1, Q_e_2).detach()
            #         std_e = log_std_e.exp()
