...
class Actor(BaseActor):
    ...
    def encode(self, mtobs: MTObs, detach: bool = False) -> TensorType:
        ...
        # print(encoding,mtobs.task_info.encoding)
    ...
    def forward_ori_mu(
        ...
        return mu, pi, log_pi, log_std,old_mu
    ...
    def forward_expert(
        ...
        mtobs2 = copy.deepcopy(mtobs)
        # mtobs = mtobs2
        # print(mtobs.task_info.env_index,mtobs.task_info.env_index==relabel_id)
        mask = (mtobs.task_info.env_index==relabel_id).float().to(mtobs.task_info.env_index.device)
        mtobs2.task_info.env_index = torch.ones_like(mtobs.task_info.env_index)*expert_id
        mask2 = (mtobs.task_info.env_index==relabel_id).float().to(mtobs.task_info.env_index.device)*(1-torch.Tensor(mtobs.info).to(mtobs.task_info.env_index.device).unsqueeze(1))
        mtobs2.task_info.env_index *= mask2.int()
        # for j in range(mtobs.env_obs.shape[0]):
        #     if mtobs.task_info.env_index[j]==relabel_id:
        #         if mtobs.info[j]['grasp_success']:
        #             mtobs2.task_info.env_index[j] = 0
        mtobs = mtobs2
        task_info = mtobs.task_info
        assert task_info is not None
        if self.should_condition_encoder_on_task_info:
        ...
        return mu.detach(), pi.detach(), log_pi.detach(), log_std.detach(), mask.detach()



    def forward_expert_auto(
        self,
        mtobs: InfoMTObs,
        detach_encoder: bool = False,
        expert_id=None,
        ...
        # print(mtobs.task_info.env_index,mtobs.task_info.env_index==relabel_id)
        # mask = (mtobs.task_info.env_index==relabel_id).float().to(mtobs.task_info.env_index.device)
        mask = torch.ones_like(mtobs.task_info.env_index)
        mtobs2.task_info.env_index = torch.ones_like(mtobs.task_info.env_index)*expert_id
        # for j in range(mtobs.env_obs.shape[0]):
        #     if mtobs.task_info.env_index[j]==relabel_id:
        #         if mtobs.info[j]['grasp_success']:
        ...
        return mu.detach(), pi.detach(), log_pi.detach(), log_std.detach(), mask.detach(), old_mu.detach()
