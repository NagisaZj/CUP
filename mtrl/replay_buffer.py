...
class ReplayBufferSample:
    ...
    infos: Optional
    ts: Optional
    expert_means: Optional
    expert_vars: Optional
...
class ReplayBuffer(object):
    ...
    def __init__(
        self, env_obs_shape, task_obs_shape, action_shape, capacity, batch_size, device,num_experts
    ):
        ...
        self.ts = np.empty((capacity, 1), dtype=np.float32)
        self.expert_means = np.empty((capacity, self.num_experts, *action_shape), dtype=np.float32)
        self.expert_vars = np.empty((capacity, self.num_experts, *action_shape), dtype=np.float32)
    ...
    def add(self, env_obs, action, reward, next_env_obs, done, task_obs,info=None,t=0,expert_mean=np.zeros([3,4]),expert_var=np.zeros([3,4])):
        ...
        self.infos[self.idx] = copy.deepcopy(info)
        self.ts[self.idx] = t
        np.copyto(self.expert_means[self.idx], expert_mean)
        np.copyto(self.expert_vars[self.idx], expert_var)
    ...
    def sample(self, index=None) -> ReplayBufferSample:
        ...
        expert_means = torch.as_tensor(self.expert_means[idxs], device=self.device)
        expert_vars = torch.as_tensor(self.expert_vars[idxs], device=self.device)
        infos = [self.infos[i] for i in idxs]
        ts = torch.as_tensor(self.ts[idxs], device=self.device)

        return ReplayBufferSample(
            env_obses, actions, rewards, next_env_obses, not_dones, env_indices, idxs,infos,ts,expert_means,expert_vars
        )
    ...
    def sample_an_index(
        ...
        return ReplayBufferSample(
            env_obses, actions, rewards, next_env_obses, not_dones, env_indices, idxs, infos
        )
