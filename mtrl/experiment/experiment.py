...
class Experiment(checkpointable.Checkpointable):
    def __init__(self, config: ConfigType, experiment_id: str = "0"):
        ...
        print(env_obs_shape,action_shape)

        self.config = prepare_config(config=self.config, env_metadata=self.env_metadata)
        import copy, pickle
        # tmpconfig = copy.deepcopy(self.config)
        # tmpconfig = pickle.dumps(tmpconfig)
        # with open('/data2/zj/NewMTRL/config_810.txt', 'ab') as f:
        #     f.write(tmpconfig)
        ...
        # self.agent.load_latest_step(model_dir='/data2/zj/NewMTRL/logs/90f2497ff4cee27c0d30fbc66e6ba205f94808ba4ea16e057df58e73_issue_None_seed_831/model')
        # self.agent.source_agent = None
        # self.agent.source_agent_2 = None
        # self.agent.source_agent_3 = None
        self.agent.source_agents = []
        if self.config.setup.load:
            if self.config.setup.load_dir != '0':
                ...
                self.source_agent.load_latest_step(model_dir=self.config.setup.load_dir)
            ...
            if self.config.setup.load_dir_2!='0':
                ...
                # self.agent.load_latest_step(model_dir=self.config.setup.load_dir_2)
                # self.agent.load_latest_step(model_dir='/data2/zj/NewMTRL/logs/90f2497ff4cee27c0d30fbc66e6ba205f94808ba4ea16e057df58e73_issue_None_seed_550/model')
                self.source_agent_2.load_latest_step(model_dir=self.config.setup.load_dir_2)
                self.source_agent_2.actor.log_std_bounds = self.config.setup.load_log_std_bounds
                self.agent.source_agents.append(self.source_agent_2)
            if self.config.setup.load_dir_3!='0':
                # with open('/data2/zj/NewMTRL/config.txt', 'rb') as f:
                #     config_tmp = pickle.load(f)
                self.source_agent_3 = hydra.utils.instantiate(
                    config_tmp.agent.builder,
                    env_obs_shape=env_obs_shape,
                    action_shape=action_shape,
                    action_range=[
                ...
                # self.agent.load_latest_step(model_dir=self.config.setup.load_dir_2)
                # self.agent.load_latest_step(model_dir='/data2/zj/NewMTRL/logs/90f2497ff4cee27c0d30fbc66e6ba205f94808ba4ea16e057df58e73_issue_None_seed_550/model')
                self.source_agent_3.load_latest_step(model_dir=self.config.setup.load_dir_3)
            ...
            #     device=self.device,
            # )
            # self.source_agent_4.load_latest_step(model_dir='/data2/zj/NewMTRL/logs/90f2497ff4cee27c0d30fbc66e6ba205f94808ba4ea16e057df58e73_issue_None_seed_810/model')
            # self.source_agent_4.actor.log_std_bounds = self.config.setup.load_log_std_bounds
        ...
        #             device=self.device,
        #         )
        #     agent.load_latest_step(
        #         model_dir=p)
        #     agent.actor.log_std_bounds = self.config.setup.load_log_std_bounds
        #     self.agent.source_agents.append(agent)
        # self.start_step = 0
